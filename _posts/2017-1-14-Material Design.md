...
![](http://i1.piimg.com/567571/69d0886845e8a659.png)
...
## DrawerLayout
抽屉布局，左右两侧可划出界面，效果同第三方SlidingMenu。
...
## TextInputLayout
输入布局，中间需要放入一个EditText。

![](http://p1.bqimg.com/567571/9a59f97c364ed4b7.png)
...
### 方法
- ``setError(CharSequence error)``：设置错误提示信息
- ``setErrorEnabled(boolean enabled)``：设置是否开启错误提示
- ``setHint(CharSequence hint)``：设置hint提示
- ``setHintEnabled(boolean enabled)``：设置是否开启hint提示
- ``getError()``：获得错误提示信息
- ``getHint()``：获得TextInputLayout中EditText的hint提示
- ``getEditText()``：获得TextInputLayout中EditText对象
...
<android.support.design.widget.TabLayout
    ...
    android:layout_width="match_parent"
    android:layout_height="wrap_content"
    android:background="#20BCD2"
    app:tabIndicatorColor="#FFFE92"
    app:tabSelectedTextColor="#FFFFFF"
    app:tabTextColor="#B7ECF1" />
```

### 方法
- ``addTab(Tab tab ,boolean setSelected)``：添加标签页，并设置是否默认选中
- ``newTab()``：生成一个标签
	- ``setText(CharSequence text)``：设置标签显示内容，该方法是Tab的方法
- ``removeTab(Tab tab)``：移除指定标签页
- ``removeTabAt(int position)``：移除指定位置的标签页
- ``removeAllTabs()``：移除所有标签页
- ``getTabAt(int index)``：获得指定位置的标签页
- ``getTabCount()``：获得标签页的总数量
- ``setupWithViewPager(ViewPager viewpager)``：与ViewPager建立关联
- ``addOnTabSelectedListener(TabLayout.OnTabSelectedListener listener)``：添加标签选择监听器